// Static journey data that always appears at the top of ePOD Approved tab
export const priorityApprovedJourneys = [
  {
    "lcuNo": "LCU95304199",
    "lrNo": "LR20257713",
...
// Transform static journey data to Journey interface format
export function transformStaticJourneyData(journeyData: typeof priorityApprovedJourneys[number]) {
  const totalAddonCharges = (journeyData.charges?.addOn?.toll || 0) + 
                            (journeyData.charges?.addOn?.unloading || 0) + 
                            (journeyData.charges?.addOn?.other || 0);

  return {
    id: `static-${journeyData.lcuNo}`,
    lcuNo: journeyData.lcuNo || 'N/A',
    journeyNo: journeyData.lcuNo || 'N/A',
    lrNo: journeyData.lrNo || 'N/A',
    transporter: journeyData.transporter || 'Unknown Transporter',
    consignor: 'Static Data',
    origin: journeyData.origin?.city && journeyData.origin?.state 
      ? `${journeyData.origin.city}, ${journeyData.origin.state}` 
      : 'Unknown',
    destination: journeyData.destination?.city && journeyData.destination?.state 
      ? `${journeyData.destination.city}, ${journeyData.destination.state}` 
      : 'Unknown',
    baseCharges: journeyData.charges?.baseFreight || 0,
    addonCharges: totalAddonCharges,
    totalAmount: journeyData.invoiceSummary?.totalPayableAmount || 0,
    status: journeyData.status === 'APPROVED' ? 'Approved' : journeyData.status === 'PENDING' ? 'Pending' : 'Missing details',
    epodStatus: journeyData.status === 'APPROVED' ? 'approved' as const : 'pending' as const,
    category: journeyData.status === 'APPROVED' ? 'ePOD Approved' as const : 'ePOD Pending' as const,
    pickupDate: journeyData.pickupDate || new Date().toISOString().split('T')[0],
    deliveryDate: journeyData.deliveryDate,
    vehicleNumber: 'N/A',
    vehicleType: "Container",
    materialType: journeyData.material?.description || 'General Cargo',
    consigneeName: journeyData.destination?.city || 'Unknown Consignee',
    paymentMode: "To-pay" as const,
    contractType: "Rate Card" as const,
    materials: journeyData.material ? [{
      id: `mat-${journeyData.lcuNo}`,
      item: journeyData.material.description || 'General Cargo',
      description: `${journeyData.material.packageCount || 0} ${journeyData.material.packageType || 'units'}`,
      quantity: journeyData.material.packageCount || 0,
      uom: journeyData.material.packageType || 'units',
      weight: journeyData.material.quantityKg || 0,
    }] : [],
    charges: [
      {
        id: `base-${journeyData.lcuNo}`,
        type: 'Base Freight',
        category: 'Base' as const,
        amount: journeyData.charges?.baseFreight || 0,
        source: 'System' as const,
        gstRate: journeyData.charges?.taxes?.sgstRate || 0.06,
        gstAmount: (journeyData.charges?.taxes?.sgstAmount || 0) + (journeyData.charges?.taxes?.cgstAmount || 0),
      },
      ...(journeyData.charges?.addOn?.toll ? [{
        id: `toll-${journeyData.lcuNo}`,
        type: 'Toll Charges',
        category: 'Add-on' as const,
        amount: journeyData.charges.addOn.toll,
        source: 'System' as const,
        gstRate: 0,
        gstAmount: 0,
      }] : []),
      ...(journeyData.charges?.addOn?.unloading ? [{
        id: `unload-${journeyData.lcuNo}`,
        type: 'Unloading Charges',
        category: 'Add-on' as const,
        amount: journeyData.charges.addOn.unloading,
        source: 'System' as const,
        gstRate: 0,
        gstAmount: 0,
      }] : []),
    ],
    podDocuments: journeyData.status === 'APPROVED' && journeyData.documents?.epodPdfPath ? [{
      id: `pod-${journeyData.lcuNo}`,
      filename: journeyData.documents.epodPdfPath.split('/').pop() || 'POD.pdf',
      uploadDate: journeyData.deliveryDate || journeyData.pickupDate || new Date().toISOString().split('T')[0],
      uploadedBy: journeyData.transporter || 'System',
      status: 'Approved' as const,
    }] : [],
    invoiceDocuments: journeyData.documents?.invoiceNumber ? [{
      id: `inv-${journeyData.lcuNo}`,
      filename: journeyData.documents.invoicePdfPath?.split('/').pop() || `${journeyData.documents.invoiceNumber}.pdf`,
      uploadDate: journeyData.deliveryDate || journeyData.pickupDate || new Date().toISOString().split('T')[0],
      uploadedBy: journeyData.transporter || 'Transporter',
      status: journeyData.status === 'APPROVED' ? 'Approved' as const : 'Pending' as const,
    }] : [],
  };
}
